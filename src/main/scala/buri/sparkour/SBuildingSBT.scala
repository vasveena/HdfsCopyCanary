...
object SBuildingSBT {
	def main(args: Array[String]) {
                var j,l = 0
                val result = (s"ls /home/ec2-user/diff1").!!
                val result2 = (s"ls /home/ec2-user/diff2").!!
                if (result.size > 0)
                {
                  j=result.split("\n").size
                }
		...
		val spark = SparkSession.builder.appName("test").getOrCreate()
                ...
                val supplier = spark.read.format("csv").option("delimiter","|").schema(customSchema).load("file:///home/ec2-user/supplier.tbl").repartition(400).cache
                ...
                {
                  j = j + 1
                  diff1.repartition(1).write.parquet("file:///home/ec2-user/diff1/folder"+j+"/")
                }
                println("Count of diff2 between two dataframes: " +diff2count)
                if (diff2count > 0)
                { 
                  l = l + 1
                  diff2.repartition(1).write.parquet("file:///home/ec2-user/diff2/folder"+l+"/")
                }
